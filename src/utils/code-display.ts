...
import { ADDED_LINE_BG, REMOVED_LINE_BG, CONTEXT_LINE_BG, ADDED_LINE_TEXT, REMOVED_LINE_TEXT, SECONDARY_COLOR, WARNING_COLOR } from '../config/colors';
 ...
 * When isContext is true, added/removed lines use subdued CONTEXT_LINE_BG instead of their normal colors.
 */
function renderRegionLines(codeContext: string[], startLineNum: number, isContext?: boolean): void {
  ...
  codeContext.forEach((line) => {
    ...
    if (line.startsWith('+')) {
      if (isContext) {
        console.log(chalk.hex(SECONDARY_COLOR)(lineNumStr + ' │ ') + chalk.bgHex(CONTEXT_LINE_BG).hex(ADDED_LINE_TEXT)(line));
      } else {
      ...
      if (isContext) {
        console.log(chalk.hex(SECONDARY_COLOR)(lineNumStr + ' │ ') + chalk.bgHex(CONTEXT_LINE_BG).hex(REMOVED_LINE_TEXT)(line));
      } else {
...
export function displayMultiRegionCodeContext(regions: CodeRegion[]): void {
  ...
  regions.forEach((region, index) => {
    renderRegionLines(region.lines, region.startLineNum, region.isContext);
