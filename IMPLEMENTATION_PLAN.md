...
- **TODO: Add `config init` command for interactive setup wizard**
...
- **NEW FEATURE: Local project guidelines and review rules**
  - Check for `.aireview` or `.ai-review.json` file in project root
  - This file contains project-specific review guidelines:
  ...
  - Load and merge with global config (project config takes precedence)
  - Include project guidelines in AI review prompts for context-aware reviews

#### 2.5 Project Guidelines Setup
- **NEW FEATURE: Prompt user to create project guidelines**
  - When running review in a project directory without `.aireview` file, detect this
  - Ask user: "No project review guidelines found. Would you like to create one?"
  - If yes, launch interactive wizard to create `.aireview` file:
    - Ask about coding standards (e.g., "Any naming conventions?")
    - Ask about architecture patterns (e.g., "MVC, microservices, etc.?")
    - Ask about specific focus areas (e.g., "Focus on security? performance?")
    - Save to `.aireview` file in project root
  - Provide option to skip and not ask again (store in global config)
  - Suggest adding `.aireview` to git repository for team-wide consistency
...
- Implement prompt sending and response parsing
- Add support for different models (GPT-4, GPT-4-turbo, etc.)
- Handle rate limiting and errors

#### 3.4 Implement Google Gemini Provider
- Implement Google Gemini API integration
- Handle API authentication
- Implement prompt sending and response parsing
- Add error handling specific to Gemini API

#### 3.5 Add Provider Factory
...
- **NEW FEATURE: Allow PR approval after review**
  - After posting comments (or choosing not to), prompt user to approve the PR
  - Option to approve even with minor suggestions/comments
  - Use GitHub API to submit PR review with approval status
  - Display choices:
    - "Approve PR" - Submit approval (requires typing "APPROVE" as confirmation)
    - "Request changes" - Mark PR as needing changes (requires typing "REQUEST CHANGES" as confirmation)
    - "Comment only" - Leave comments without approval status (default)
    - "Skip" - Don't submit any review status
  - **Safety confirmation**: For "Approve" and "Request changes", require user to type the action name (e.g., "APPROVE") to confirm
  - Combine comments + approval in a single GitHub review submission for better UX
  - Show confirmation message with review status and link to PR
...
- Add `--format` flag for output format (interactive, json, markdown)
- **TODO: Improve help output to show all commands with options**
  - When user runs `ai-review --help`, show comprehensive help including:
    - All available commands (init, config, pr, demo)
    - Options for each command with clear descriptions
    - Examples:
      - `pr [id]` - Review pull requests
        - `--post`: Automatically post accepted comments without confirmation
        - `--dry-run`: Preview what comments would be posted without actually posting them
      - `config <action>` - Manage configuration (set, get, list, delete)
      - `init` - Interactive setup wizard
  - Since we don't have many commands yet, showing everything upfront is better UX than requiring multiple `--help` calls
...
- Test AI provider abstraction and implementations
- Test platform adapter interface
- Test diff parsing logic
- Test prompt generation

#### 11.2 Integration Tests
- Test GitHub integration with mock `gh` CLI responses
- Test end-to-end flow with mocked APIs
- Test comment posting workflow

#### 11.3 Manual Testing Checklist
- Test on real GitHub repository
- Test with different AI providers
- Test error scenarios (network failures, invalid PRs, etc.)
...
### Phase 1 (GitHub)
- [ ] Tool successfully lists open PRs from GitHub
- [ ] Tool performs AI-powered review with at least one provider (Claude/GPT/Gemini)
- [ ] Tool displays review comments interactively
- [ ] Tool posts comments back to GitHub PR
- [ ] Clear error messages for common issues
- [ ] Documentation covers installation and basic usage
- [ ] Published to at least one package manager

### Phase 2 (GitLab)
- [ ] Tool works with GitLab repositories
- [ ] All Phase 1 features work with GitLab

### Phase 3 (Bitbucket)
- [ ] Tool works with Bitbucket repositories
- [ ] All Phase 1 features work with Bitbucket

---

## Notes
- Keep architecture extensible for future platforms (Azure DevOps, Gitea, etc.)
- Prioritize user experience and clear feedback
- Focus on code quality and maintainability
- Keep dependencies minimal
- Consider security implications of handling API keys
